...
class EstabelecimentosController extends AppController
{
    ...
    public function add()
    {
	    ...
	    $estabelecimento['senha'] = crypt($estabelecimento['senha'],'$2a$'.'08'.'$'.'Cf1f11ePArKlBJomM0F6aJ'.'$');
   ...
   public function login()
    {
    	if ($this->request->is('post')) {
        	...
        	$estabelecimentos = $this->Auth->identify();
        	if ($estabelecimentos) {
        	    $this->Auth->setUser($estabelecimentos);
        	    return $this->redirect(['controller' => 'filapedidos', 'action' => 'index']);
        	}
